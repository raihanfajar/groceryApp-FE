...
export interface StoreProductCard {
  ...
  discount: null | {
    type: "MANUAL" | "MINIMUM_PURCHASE" | "BOGO" | "REGULAR";
...
const fetchAllTargetStoreProducts = async (
  storeId: string | null | undefined,
) => {
  // call your backend route
  const { data } = await axiosInstance.get<getAllTargetStoreProductsResponse>(
    `/stores/products/${storeId}`,
  );
  return data.data;
};

export const useGetAllTargetStoreProducts = (
  storeId: string | null | undefined,
) => {
  return useQuery({
