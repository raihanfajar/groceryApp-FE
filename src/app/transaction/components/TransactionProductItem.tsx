...
function TransactionProductItem({ item }: { item: TransactionProduct }) {
  ...
  return (
    <>
      {/* Desktop View */}
      <div className="mt-2 hidden items-center rounded-lg border-b-2 bg-[#f5f5f5] p-6 text-gray-800 md:flex">
        ...
        <div className="text-l w-3/12 text-center">
          {formatCurrency(finalPrice)}
        </div>
      </div>

      {/* Mobile View */}
      <div className="w-full border-b py-3 md:hidden bg-[#f5f5f5] rounded-lg mt-2">
        <div className="flex items-start gap-4 px-1 pt-1 pb-2">
          ...
          <div className="flex flex-1 flex-col gap-1">
            ...
            <div className="mt-2 ml-auto flex items-baseline gap-4">
              ...
              <span className="font-lg">{formatCurrency(finalPrice)}</span>
            </div>
          </div>
        </div>
      </div>
