...
const CartRow = ({ item, isUpdating, onUpdateQuantity, onPromptDelete }: CartRowProps) => {
  const { product, quantity, activePrice, discountAmount, storeId, availability } = item;
  const basePrice = activePrice + discountAmount;

  // Variabel untuk mengecek ketersediaan dengan mudah
  const isAvailable = availability.status === "AVAILABLE";

  const handleUpdate = (newQuantity: number) => {
    if (isUpdating) return;
    // Blokir penambahan kuantitas jika stok tidak cukup
    if (newQuantity > availability.currentStock) {
  ...
  return (
    <>
      {/* Desktop View */}
      <div className={`text-secondary mt-2 hidden items-center border-b-2 py-2 md:flex ${!isAvailable ? 'bg-gray-50 text-gray-500' : ''}`}>
        <div className="flex w-4/12 items-center gap-10">
          ...
          <div className="flex flex-col">
            ...
            {!isAvailable && (
                <div className="text-xs text-red-500 font-semibold mt-1">Not enough stock !</div>
            )}
          </div>
        </div>
          ...
          {isAvailable ? (
            <div className="flex items-center">
              ...
              <button
                type="button"
                onClick={() => handleUpdate(quantity + 1)}
                disabled={isUpdating || quantity >= availability.currentStock}
        ...
        <div className="flex w-1/12 items-center justify-center">
          {!isAvailable && (
            <button onClick={() => onPromptDelete(item)} className="text-red-500 hover:text-red-700" title="Hapus item">
      ...
      {/* Mobile View */}
      <div className={`w-full md:hidden border-b py-2 ${!isAvailable ? 'bg-gray-50 text-gray-500' : ''}`}>
        <div className="flex items-start gap-4 px-1 pt-1 pb-2">
          ...
          <div className="flex flex-1 flex-col gap-1">
            ...
            <div className="items-center gap-5">
              {discountAmount > 0 && (
                <div className="text-sm line-through">{formatCurrency(basePrice)}</div>
              )}
